...
TFrame10 *Frame10;
//---------------------------------------------------------------------------
__fastcall TFrame10::TFrame10(TComponent* Owner, String baseurl, String resource, String titulo)
	: TFrame1(Owner, baseurl, resource, titulo), baseurl(baseurl), resource(resource), titulo(titulo)
{
	RellenarComboBox(ComboBoxMarca, "switchmodelomarca");
	RellenarComboBox(ComboBoxTipo, "switchmodelotipo");
	PopulateStringGrid();
	ButtonEdit->Enabled = false;
}
//---------------------------------------------------------------------------
void __fastcall TFrame10::ButtonCloseClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void TFrame10::PopulateStringGrid()
{
		...
		if (RESTRequest->Response->StatusCode == 200) {
			...
			for (int i = 0; i < JSONArray->Count; ++i) {
				...
				if (JSONObject) {
					...
					TJSONValue* JSONObjectMarca = JSONObject->GetValue("switchmodelomarca_id");
					TJSONValue* JSONObjectTipo = JSONObject->GetValue("switchmodelotipo_id");
...
//---------------------------------------------------------------------------
void TFrame10::RellenarComboBox(TComboBox *ComboBox, const UnicodeString &resource)
{
...
void __fastcall TFrame10::StringGridSelectCell(TObject *Sender, const int ACol,
...
void __fastcall TFrame10::ButtonNewClick(TObject *Sender)
{
    NewRecord();
}
//---------------------------------------------------------------------------
void TFrame10::NewRecord()
{
	LabelId->Text = "";
	EditClave->Text = "";
	MemoDescr->Text = "";
...
void __fastcall TFrame10::ButtonDeleteClick(TObject *Sender)
{
	DeleteRecord();
}

void TFrame10::DeleteRecord()
{
	TRESTClient *RESTClient = new TRESTClient(NULL);
	TRESTRequest *RESTRequest = new TRESTRequest(NULL);
...
void __fastcall TFrame10::ButtonEditClick(TObject *Sender)
{
...
void __fastcall TFrame10::ButtonSaveClick(TObject *Sender)
{
...
void TFrame10::UpdateRecord()
{
	...
	RESTRequest->Response = RESTResponse;
	RESTRequest->Client = RESTClient;
	RESTRequest->Resource = resource+"?id="+LabelId->Text+"&clave="+EditClave->Text+"&descr="+MemoDescr->Text+"&switchmodelomarca_id="+marcaSeleccionada->Tag+"&switchmodelotipo_id="+tipoSeleccionado->Tag;;
	RESTRequest->Response->ContentType = "application/json";
	RESTRequest->Response->ContentEncoding = "UTF-8";
...
void TFrame10::CreateRecord()
{
	...
	if ((ComboBoxMarca->ItemIndex != -1) and (ComboBoxTipo->ItemIndex !=-1))
	{
		...
		RESTRequest->Response = RESTResponse;
		RESTRequest->Client = RESTClient;
		RESTRequest->Resource = resource+"?clave="+EditClave->Text+"&descr="+MemoDescr->Text+"&switchmodelomarca_id="+marcaSeleccionada->Tag+"&switchmodelotipo_id="+tipoSeleccionado->Tag;
		RESTRequest->Response->ContentType = "application/json";
		RESTRequest->Response->ContentEncoding = "UTF-8";
...
void __fastcall TFrame10::ComboBoxMarcaChange(TObject *Sender)
{
...
void __fastcall TFrame10::ComboBoxTipoChange(TObject *Sender)
{
    TComboBox *comboBox = dynamic_cast<TComboBox*>(Sender);
	if (comboBox != nullptr && comboBox->ItemIndex != -1)
	{
